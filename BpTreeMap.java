...
//-----------------------------------------------------------------------------------
// Retrieve values or ranges (sub-trees)
//-----------------------------------------------------------------------------------
    ...
    public Set <Map.Entry <K, V>> entrySet ()
    {
        ...
        for (var n = firstLeaf; n != null; n = (Node) n.ref[0]) {
            for (var i = 0; i < n.keys; i++)
                enSet.add (new AbstractMap.SimpleEntry<K, V> (n.key[i], (V) n.ref[i]));
        } // for
        return enSet;
    } // entrySet
...
//-----------------------------------------------------------------------------------
    ...
    private Node insert (K key, V ref, Node n)
    {
        ...
        if (n.isLeaf) {                                               // handle LEAF node level
            ...
            if (rt != null) {
                K divKey = rt.key[0];
                n.add(divKey, rt);
                if (n.overflow()) { // generate new sibling if the added key induces an overflow
                    Node newSibling = n.splitI();
                    if (n != root) {
                        return newSibling;
                    }
                    root = new Node (root, newSibling.key[0], newSibling);
                }
            }
        } // if
    ...
    {
        Node rt = null;
        int location = n.find(k);
        for (int i = n.keys; i > location; i--){ // Moves keys and references to make room for new key
            n.key[i] = n.key[i - 1];
            n.ref[i + 1] = n.ref[i];
        }

        n.key[location] = k; // new key
        n.ref[location + 1] = v; // new reference
        n.keys++; // new key count

        if (n.overflow()) { // If the tree is now full/overflowed it will split the internal node
            return n.splitI();
        }
...
//-----------------------------------------------------------------------------------
    ...
    public static void main (String [] args)
    {
        ...
        if (RANDOMLY) {
            ...
            for (var i = 1; i <= totalKeys; i += 2) bpTree.put (rng.nextInt (2 * totalKeys), i * i);
        } else {
        ...
        out.println ("-------------------------------------------");
        out.println (STR."number of keys in BpTree = \{bpTree.kCount}");
        out.println ("-------------------------------------------");
        out.println ("Average number of nodes accessed = " + bpTree.count / (double) totalKeys);
